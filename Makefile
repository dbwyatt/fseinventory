...
run: rm
	...
	docker run -d -p 8001:8000 --network=$(NETWORK) --network-alias myapp -v $(APPLICATION_PATH):/app -e CODEIGNITER_PROJECT_NAME=fseinventory -e CI_ENV=testing --name $(APP_CONTAINER) bitnami/codeigniter:latest
